...
void Server::commandJOIN(Client& client, Message& msg)
{
	...
	if (msg.params.empty())
	{	
		sendReply(client, generateErrorReply(_servername, ERR_NEEDMOREPARAMS, "JOIN"));
		return ;
	}
	...
	for (std::vector<std::string>::iterator it = channels.begin(); it != channels.end(); it++, j++)
	{
		if ((channel = find_channel(*it)))
		{
			...
			{
				sendReply(client, generateErrorReply(_servername, ERR_TOOMANYCHANNELS, *it));
				continue;
			}
			if (channel->get_invite_status() && !client.check_invitation(*it))
			{
				sendReply(client, generateErrorReply(_servername, ERR_INVITEONLYCHAN, *it));
				continue;
			}
			if (channel->check_bans(client))
			{
				sendReply(client, generateErrorReply(_servername, ERR_BANNEDFROMCHAN, *it));
				continue;
			}
			if (channel->have_key() && (keys.size() < j || !(channel->check_key(keys[j]))))
			{	
				sendReply(client, generateErrorReply(_servername, ERR_BADCHANNELKEY, *it));
				continue;
			}
			if (!(channel->under_user_limit()))
			{
				sendReply(client, generateErrorReply(_servername, ERR_CHANNELISFULL, *it));
				continue;
			}
			channel->add_member(&client);
			if (!(channel->get_topic().empty()))
				sendReply(client, generateNormalReply(_servername, RPL_TOPIC, *it, channel->get_topic()));
			else
				sendReply(client, generateNormalReply(_servername, RPL_NOTOPIC, *it));
			Message  mes;
		...
		else
		{
			...
			{	
				sendReply(client, generateErrorReply(_servername, ERR_NOSUCHCHANNEL, *it));
				continue;
			}
			if (!client.under_channels_limit())
			{
...
void	Server::commandPART(Client &client, Message &msg)
{
	...
	if (msg.params.empty())
	{	
		sendReply(client, generateErrorReply(_servername, ERR_NEEDMOREPARAMS, "PART"));
		return ;
	}
	std::vector<std::string>	channels;
	divide_comma(channels, msg.params[0]);
	...
	for (std::vector<std::string>::iterator it = channels.begin(); it != channels.end(); it++)
	{
		if ((channel = find_channel(*it)))
		{
			if (!(channel->have_member(client)))
			{	
				sendReply(client, generateErrorReply(_servername, ERR_NOTONCHANNEL, *it));
				continue;
			}
